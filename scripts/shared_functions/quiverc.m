...
% Extended by Stephan Jonas 9/14/12
...
%   QUIVERC(U,V,C) plots velocity vectors at equally spaced points in
%   the x-y plane colorcoded with the colors givin in C.
%
%   QUIVERC(U,V,S), QUIVERC(U,V,C,S), QUIVER(X,Y,U,V,S) or QUIVER(X,Y,U,V,C, S) automatically scales the 
...
if length(varargin{nin})==1 % quiver(u,v,s) or quiver(x,y,u,v,s)
  autoscale = varargin{nin};
    nin = nin-1;
end

% Check numeric input arguments
if nin<4 % quiver(u,v), quiver(u,v,c) or quiver(u,v,c,s)
